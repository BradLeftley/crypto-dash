...
import { Row, Form, Col, Button, Modal, Select, InputNumber, Input, Radio } from 'antd';
...
const createPriceCard = (data) => {
    ...
    <Col xs={16} sm={12} md={8} lg={8} xl={4}>
        <PriceCard  name={item.name} marketSymbol={item.id} coingecko targets={item.target}/>
    </Col>)
}
...
function PriceCardList() {
    ...
    const { loading, error, data } =  useFetch(`https://api.coingecko.com/api/v3/coins/list`)
    ...
    const showModal = () => {
      ...
      const handleOk = () => {
        ...
        .then(values => {
         ...
         setCoins([...coins,{id: values.coinId, name: coin[0].name, target: [values.target1 || 0 ,values.target2 || 0,values.target3 || 0]}])
        ...
        <>
            ...
            <Button type="primary" shape="circle" icon={<PlusOutlined />} size='Large' onClick={showModal} />
                <Modal title="Basic Modal" visible={isModalVisible} onOk={handleOk} onCancel={handleCancel}>
                    ...
                    >
                    <Select showSearch optionFilterProp="children" placeholder='search...' allowClear>
                        {selectFields}
                    </Select>
                    </Form.Item>
