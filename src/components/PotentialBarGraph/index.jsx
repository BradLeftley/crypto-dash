...
function PotentialBarGraph(props) {
  ...
  const { loading, error, data } = useFetch(
    `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=${listCoinIds(coins).toString()}`)
  if (loading) return <p>loading...</p>;
  if (error) return <p>Error :(</p>;

    let filteredData = [];
    itemSelected.forEach(item => filteredData.push(...coins.filter(filter => item === filter.name)));
  ...
  return (
   <Card title="Percentage to Target Price">
     <Radio.Group value={targetValue} onChange={(e)=>{setTargetValue(e.target.value)}}>
       ...
       <BarGraph data={getCoinsGraphData(filteredData,targetValue,data)} rowName='name' columnName='percentage'/> 
    </Card>
  );
};
